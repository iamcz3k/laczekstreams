// ISO 3166-1 alpha-2 codes used to build flag image URLs via flagcdn.com.
// Covers every country and most dependent territories — admin can search by name.
...
export const COUNTRIES: Country[] = [
  ...
  // Football-relevant sub-nations
  { code: "gb-eng", name: "England" }, { code: "gb-sct", name: "Scotland" }, { code: "gb-wls", name: "Wales" },
  { code: "gb-nir", name: "Northern Ireland" },
];

/** Returns a flag image URL (or null when value isn't an ISO code we know). */
export function flagUrl(codeOrUrl?: string | null, width: 40 | 80 | 160 = 80): string | null {
  if (!codeOrUrl) return null;
  if (codeOrUrl.startsWith("http")) return codeOrUrl;
  const code = codeOrUrl.toLowerCase();
